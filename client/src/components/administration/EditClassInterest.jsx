...
export default function EditClassInterest() {
  ...
  const onSubmit = (data) => {
    ...
    formData.append(
      "targetGroup",
      JSON.stringify(Array.isArray(targetGroups) ? targetGroups : [])
    );

    axiosClient
      .put(`/activityInterest/updateInterest/${id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      .then(() => axiosClient.get(`/activityInterest/getEveryInterest/`))
      .then((response) => {
              ...
              >
                ...
                <div className="grid text-sm grid-cols-3 gap-1">
                  {Object.entries(allTargetGroups).map(([imageUrl, name]) => (
                    <label key={imageUrl} className="flex items-center gap-2">
                      <input
                        type="checkbox"
